...
class PathLocks {
  ...
  private readonly queue = new Map<string, Array<{ grant: () => void; reject: (error: Error) => void }>>();

  async withLock<T>(key: string, fn: () => Promise<T>, waitMs = 10000): Promise<T> {
  ...
  private acquire(key: string, waitMs: number): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        if (current.length === 0) this.queue.delete(key);
  ...
  private release(key: string): void {
    ...
    if (waiters.length === 0) this.queue.delete(key);
    if (next !== undefined) {
      next.grant();
      return;
    }
...
function keepBom(text: string): { text: string; bom: boolean } {
  return text.startsWith('\uFEFF') ? { text, bom: true } : { text, bom: false };
}

/**
 * Read one text file completely and provably.
 *
 * The whole file is read through a SINGLE file descriptor, but only when it fits
 * inside `readMaxWindowBytes`. Files larger than that are refused with an
 * explicit FILE_TOO_LARGE instead of being silently cut: paginating inside a
 * huge file was what made the previous design both wrong (the byte budget could
 * make a later range unreachable) and unprovable (the version could describe a
 * different revision than the content).
 *
 * Identity is verified, not assumed: the file's size, mtime, ctime, device and
 * inode are compared before and after the read. If the file moved, grew or was
 * replaced while we read it, the read is refused with VERSION_CONFLICT rather
 * than returning a mix of two revisions. Content and the full sha256 come from
 * the same buffer, so a caller can never receive a partial hash as a whole-file
 * version.
 */
...
async function readWholeFileStable(path: string, maxBytes: number): Promise<StableRead> {
  let handle;
  try {
  ...
  try {
    ...
    if (before.size > maxBytes) {
      throw new DirectOpsError(
        'FILE_TOO_LARGE',
        'file is larger than the configured read window; this surface reads a whole file so that the content and '
          + 'its version are provably the same revision, and refuses rather than returning a partial file',
        { size: before.size, limit: maxBytes, path: scrubPathForDisplay(path) },
      );
    }
    ...
    if (!unchanged) {
      throw new DirectOpsError(
        'VERSION_CONFLICT',
        'file changed while it was being read (size, mtime, ctime, device or inode differ); re-read to get a '
...
function versionOfCompleteFile(buffer: Buffer, size: number, mtimeMs: number): FileVersionInfo {
  return {
...
export async function readTextFile(input: ReadInput, policy: DirectOpsPolicy): Promise<ReadResult> {
  ...
  const budget = Math.min(
    input.max_bytes !== undefined && input.max_bytes > 0 ? input.max_bytes : policy.limits.readMaxBytes,
    policy.limits.readMaxBytes,
  );
  ...
  if (startLine > totalLines) {
    throw new DirectOpsError(
      'INVALID_ARGUMENT',
      `start_line ${startLine} is past the end of the file (${totalLines} lines)`,
      { path: scrubPathForDisplay(target.canonical), line_count: totalLines },
    );
  }
  ...
  for (let index = startLine - 1; index < totalLines; index += 1) {
    ...
    if (bytesUsed + lineBytes > budget) break;
    bytesUsed += lineBytes;
    lines.push(line);
    endLine = index + 1;
  }
  ...
  const truncation: ReadResult['truncation'] = lineTooLong
    ? 'line_too_long'
    : endLine === 0
      ? 'byte_budget'
      : lineWindowCapped
 ...
 */
async function verifyCommittedContent(
  path: string,
  expectedSha256: string,
  expectedSize: number,
): Promise<{ sha256: string; size: number; mtimeMs: number }> {
  ...
  try {
    ...
    if (actual !== expectedSha256) {
      throw postCommitConflict(
        path,
        'the file no longer holds the content this call wrote (a same-length replacement is detected here); the '
          + 'write did happen, so this is reported as an unverified commit rather than a failed one',
...
function postCommitConflict(path: string, message: string, details: Record<string, unknown>): DirectOpsError {
  return new DirectOpsError(
    'POST_COMMIT_CONFLICT',
    `${message}. The write already happened; do not blindly retry — read the file to see what is there now.`,
    { path: scrubPathForDisplay(path), committed: true, ...details },
  );
}

async function currentVersion(path: string, maxBytes: number): Promise<FileVersionInfo> {
  const { buffer, size, mtimeMs } = await readWholeFileStable(path, maxBytes);
  return versionOfCompleteFile(buffer, size, mtimeMs);
}

function assertVersionMatch(expected: string, actual: FileVersionInfo, path: string): void {
  if (expected !== actual.sha256) {
    throw new DirectOpsError(
      'VERSION_CONFLICT',
      'file changed since it was read; re-read it and retry with the current file_version.sha256'
        + ' (this guard is what stops ChatGPT and a DSH agent from overwriting each other)',
      { path: scrubPathForDisplay(path), expected_sha256: expected, actual_sha256: actual.sha256 },
    );
  }
}
...
async function pathMode(path: string): Promise<number | undefined> {
  try {
...
async function commitContent(
  ...
  options: {
    ...
    mode: number;
    /**
     * Re-checked after the temp write, immediately before the commit. It must
     * also confirm the path still resolves to the same canonical file, so a
     * redirected path is refused instead of being written and then read back.
     */
    verify?: (canonical: string) => Promise<void>;
  },
): Promise<{ size: number; version: FileVersionInfo; mode: number }> {
  ...
  try {
    ...
    if (options.verify !== undefined) await options.verify(path);
    ...
    if (options.noClobber) {
      try {
        ...
        if ((error as NodeJS.ErrnoException).code === 'EEXIST') {
          throw new DirectOpsError(
            'WRITE_CONFLICT',
            'the target appeared while this create was in flight; refusing to replace it'
              + ' (use mode=overwrite with expected_sha256 after re-reading)',
            { path: scrubPathForDisplay(path) },
          );
        }
        throw error;
    ...
    throw new DirectOpsError('INTERNAL', `write failed: ${(error as Error).message}`, {
      path: scrubPathForDisplay(path),
    });
  }
...
export async function writeTextFile(input: WriteInput, policy: DirectOpsPolicy): Promise<WriteResult> {
  const started = Date.now();
  if (!policy.enabled) {
    throw new DirectOpsError('DIRECT_OPS_DISABLED', 'direct operations are disabled by the host configuration');
  }
  if (!policy.writesEnabled) {
    throw new DirectOpsError(
      'WRITE_DISABLED',
      'direct writes are disabled: no trusted root is configured as writable, or allowWrites is false',
      { writable_roots: policy.roots.filter((root) => root.writable).map((root) => root.label) },
    );
  }
  ...
  return pathLocks.withLock(target.canonical, async () => {
    ...
    if (mode === 'create' && existsNow) {
      throw new DirectOpsError(
        'WRITE_CONFLICT',
        'mode=create refuses to touch an existing file; read it and use mode=overwrite with expected_sha256 instead',
    ...
    if (mode === 'overwrite' && !existsNow) {
      throw new DirectOpsError(
        'NOT_FOUND',
        'mode=overwrite requires an existing file; use mode=create for a new file',
        { path: scrubPathForDisplay(confirmed.canonical) },
      );
    }
    if (mode === 'overwrite' && (input.expected_sha256 === undefined || input.expected_sha256 === '')) {
      throw new DirectOpsError(
        'READ_REQUIRED',
        'overwriting an existing file requires expected_sha256 from a read of the current content; this is what '
          + 'prevents overwriting work an agent or another tool did in the meantime',
        { path: scrubPathForDisplay(confirmed.canonical) },
      );
    }
    ...
    const wrote = await commitContent(confirmed.canonical, input.content, {
      ...
      verify: async (canonical) => {
        ...
        if (atCommit.canonical !== canonical || atCommit.canonical !== confirmed.canonical) {
          throw new DirectOpsError(
            'PATH_REDIRECTED',
            'the target path now resolves to a different file than when this operation started; refusing to write '
              + 'through a path that was redirected underneath it',
            { path: scrubPathForDisplay(canonical), resolved_now: scrubPathForDisplay(atCommit.canonical) },
          );
        }
        if (mode === 'create') {
          if (atCommit.exists) {
        ...
        assertVersionMatch(input.expected_sha256 as string, current, atCommit.canonical);
      },
    });
    return {
...
export async function editTextFile(input: EditInput, policy: DirectOpsPolicy): Promise<EditResult> {
  const started = Date.now();
  if (!policy.enabled) {
    throw new DirectOpsError('DIRECT_OPS_DISABLED', 'direct operations are disabled by the host configuration');
  }
  ...
  if (input.expected_sha256 === undefined || input.expected_sha256 === '') {
    throw new DirectOpsError(
      'READ_REQUIRED',
      'editing requires expected_sha256 from a read of the current content; a blind edit can silently apply to a '
        + 'revision the caller never saw',
      { path: scrubPathForDisplay(input.path) },
    );
  }
  ...
  return pathLocks.withLock(target.canonical, async () => {
    ...
    assertVersionMatch(input.expected_sha256 as string, before, confirmed.canonical);
    ...
    if (!input.replace_all && occurrences > 1) {
      throw new DirectOpsError(
        'EDIT_AMBIGUOUS',
        'old_text occurs more than once; add surrounding context or set replace_all=true',
        { path: scrubPathForDisplay(confirmed.canonical), occurrences },
      );
    }
    ...
    const wrote = await commitContent(confirmed.canonical, next, {
      ...
      verify: async (canonical) => {
        ...
        if (atCommit.canonical !== canonical || atCommit.canonical !== confirmed.canonical) {
          throw new DirectOpsError(
            'PATH_REDIRECTED',
            'the target path now resolves to a different file than when this edit started; refusing to write '
              + 'through a path that was redirected underneath it',
            { path: scrubPathForDisplay(canonical), resolved_now: scrubPathForDisplay(atCommit.canonical) },
          );
        }
...
function lineOfFirstOccurrence(text: string, needle: string): number | undefined {
...
export async function versionOfFile(path: string, policy: DirectOpsPolicy): Promise<FileVersionInfo> {
