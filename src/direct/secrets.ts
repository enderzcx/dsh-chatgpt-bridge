 ...
 */
export const DEFAULT_DENIED_SEGMENTS: string[] = [
  '.ssh',
  '.aws',
  '.gnupg',
  '.gpg',
...
export const DEFAULT_DENIED_BASENAMES: RegExp[] = [
  /^\.env(\..+)?$/i,
  /^\.credentials.*$/i,
  /^credentials(\.|$)/i,
...
export function scrubPathForDisplay(path: string): string {
 ...
 */
export function assertNotCredentialFile(canonicalPath: string, deniedBasenames: RegExp[]): void {
  ...
  for (const pattern of deniedBasenames) {
    ...
    if (pattern.test(base)) {
      throw new DirectOpsError(
        'PATH_DENIED',
        `refusing to touch a credential-shaped file name (matched ${String(pattern)})`,
