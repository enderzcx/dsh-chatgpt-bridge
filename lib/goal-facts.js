...
export function extractCommand(args) {
    ...
    if (Array.isArray(commands)) {
        const joined = commands.filter((item) => typeof item === 'string').join('\n');
        if (joined.trim() !== '')
            return joined;
    }
    return undefined;
...
export function extractFilePaths(args) {
    const paths = [];
...
export function classifyCommand(command) {
    const kinds = [];
    if (/\bgit\s+worktree\s+add\b/i.test(command))
        kinds.push('git_worktree_add');
    if (/\bnpm\s+publish\b/i.test(command))
        kinds.push('npm_publish');
    if (/\bnpm\s+pack\b/i.test(command))
        kinds.push('npm_pack');
    if (/\bgh\s+release\b/i.test(command))
        kinds.push('github_release');
...
export function parseWorktreeAddPath(command) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (token.startsWith('-')) {
            ...
            if (WORKTREE_VALUE_OPTS.has(opt) && !token.includes('='))
                i += 1;
            continue;
        }
...
export function parseMkdirPaths(command) {
    if (!/\bmkdir\b/i.test(command))
        return [];
...
function resultTextOf(data) {
    ...
    if (typeof content === 'string')
        return content.slice(0, RESULT_TEXT_CAP);
...
export function foldGoalFacts(events) {
    const tools = [];
    const open = new Map();
    let todos;
    let lastTurnReason;
...
export function factLooksLikeNpm2fa(fact) {
    if (!fact.kinds.includes('npm_publish'))
        return false;
    const code = (fact.errorCode ?? '').toUpperCase();
    if (NPM_2FA_CODES.some((item) => code.includes(item)))
        return true;
...
export function successfulKinds(facts) {
    const ok = new Set();
...
export function commandForCall(events, callId) {
    if (events === undefined || callId === undefined)
        return undefined;
    for (const event of events) {
        if (event.type !== 'tool/call')
            continue;
        const data = event.data;
        if (data === undefined || data.callId !== callId)
            continue;
...
export function filePathsForCall(events, callId) {
    if (events === undefined || callId === undefined)
        return [];
    for (const event of events) {
        if (event.type !== 'tool/call')
...
export function changedFileCountOf(events) {
    if (events === undefined)
        return 0;
    const seen = new Set();
    for (const event of events) {
