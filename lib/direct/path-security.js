...
function assertNoDeniedSegment(canonicalPath, policy) {
...
function assertAbsolute(input) {
    if (typeof input !== 'string' || input.trim() === '') {
        throw new DirectOpsError('INVALID_PATH', 'path must be a non-empty absolute path');
    }
    if (!isAbsolute(input)) {
        throw new DirectOpsError('INVALID_PATH', 'path must be absolute; relative paths are resolved against an implicit cwd and are not accepted', { path: scrubPathForDisplay(input) });
    }
}
 ...
 * withheld.
 */
function pickRoot(canonical, roots) {
    let best;
 ...
 */
function assertNotProtected(canonicalPath, policy, verb) {
    for (const protectedPath of policy.protectedPaths) {
        if (canonicalPath === protectedPath || isContained(canonicalPath, protectedPath)) {
            throw new DirectOpsError('PATH_DENIED', `${verb} is refused: this path is the direct-operations control configuration, which the direct surface `
                + 'never reads or writes because it governs the surface itself', { path: scrubPathForDisplay(canonicalPath) });
        }
    }
}
/**
 ...
 * those paths at all.
 */
export function resolveExistingTarget(input, policy, options = {}) {
    ...
    if (options.skipProtection !== true)
        assertNotProtected(canonical, policy, 'reading this path');
    const root = pickRoot(canonical, policy.roots);
    if (root === undefined) {
        throw new DirectOpsError('PATH_OUTSIDE_ROOTS', 'path is outside every trusted root of this bridge (roots are server-side configuration, not caller arguments)', { path: scrubPathForDisplay(canonical), roots: policy.roots.map((item) => item.label) });
    }
    return { canonical, root, exists: true, isSymlink: lstat.isSymbolicLink() };
}
/** Resolve a path that may be created or replaced. */
export function resolveWritableTarget(input, policy) {
    assertAbsolute(input);
    const absolute = resolve(input);
    ...
    if (existed) {
        ...
        if (root === undefined || !root.writable) {
            throw new DirectOpsError('PATH_OUTSIDE_ROOTS', root === undefined
                ? 'path is outside every trusted root of this bridge'
                : 'that trusted root is mounted read-only', { path: scrubPathForDisplay(canonical), roots: policy.roots.map((item) => item.label) });
        }
