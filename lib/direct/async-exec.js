 ...
 */
export async function startCommand(input, policy) {
    if (!policy.enabled) {
    ...
    if (policy.exec.backend !== 'codex-app-server') {
        throw new DirectOpsError('ASYNC_UNSUPPORTED_BACKEND', `async runs require exec.backend="codex-app-server" (currently ${JSON.stringify(policy.exec.backend)}); the `
            + 'local sandbox-exec path cannot stream or terminate a command after the call returns, and this surface '
            + 'will not pretend to.', { backend: policy.exec.backend });
    }
...
function createClient(policy, onDelta) {
    ...
    if (binPath === undefined || binPath.trim() === '') {
        throw new DirectOpsError('CODEX_BIN_UNCONFIGURED', 'exec.backend is "codex-app-server" but exec.codexBin is not set; a local administrator must name the '
            + 'codex executable. The bridge never guesses it from PATH.', { backend: policy.exec.backend });
    }
...
export function readRun(runId, sinceSeq, policy) {
 ...
 */
export async function terminateRun(runId, policy) {
...
export function asyncRunsSupported(policy) {
