...
export class CodexAppServerClient {
    ...
    async start() {
        if (this.child !== undefined)
            throw new CodexClientError('CODEX_ALREADY_STARTED', 'client already started');
        let child;
        try {
            ...
            throw new CodexClientError('CODEX_SPAWN_FAILED', `could not start the codex app server: ${error instanceof Error ? error.message : String(error)}`, { bin: this.opts.binPath });
        }
        this.child = child;
        ...
        child.on('exit', (code, signal) => {
            this.closed = true;
            this.failAll(new CodexClientError('CODEX_EXITED', `codex app server exited (code ${String(code)}, signal ${String(signal)})`, { exit_code: code, signal }));
        });
    ...
    onLine(line) {
        ...
        if (typeof message.id === 'number' && this.pending.has(message.id)) {
            ...
            if (message.error !== undefined) {
                request.reject(new CodexClientError('CODEX_RPC_ERROR', `codex app server rejected ${request.method}: ${JSON.stringify(message.error)}`, { method: request.method, error: message.error }));
            }
            else {
    ...
    onNotification(method, params) {
        ...
        if (payload === undefined || typeof payload.processId !== 'string')
            return;
    ...
    request(method, params, timeoutMs) {
        if (this.closed)
            return Promise.reject(new CodexClientError('CODEX_NOT_RUNNING', 'codex app server is not running'));
        if (!ALLOWED_METHODS.has(method)) {
            ...
            return Promise.reject(new CodexClientError('CODEX_METHOD_NOT_ALLOWED', `${method} is not part of the command/exec surface this client is allowed to use`, { method }));
        }
        ...
        return new Promise((resolve, reject) => {
            const timer = setTimeout(() => {
                this.pending.delete(id);
                reject(new CodexClientError('CODEX_REQUEST_TIMEOUT', `codex app server did not answer ${method} within ${timeoutMs}ms; stderr tail: ${this.stderrTail()}`, { method, timeout_ms: timeoutMs }));
            }, timeoutMs);
    ...
    async exec(request) {
        ...
        this.opts.onOutputDelta = (delta) => {
            ...
            if (delta.stream === 'stderr') {
                ...
                if (added.text.length !== decoded.length)
        ...
        if (request.outputBytesCap !== undefined)
            params.outputBytesCap = request.outputBytesCap;
        ...
        if (request.disableTimeout !== undefined)
            params.disableTimeout = request.disableTimeout;
        ...
        let result;
        try {
            result = await this.request('command/exec', params, waitMs);
        }
        finally {
        ...
        if (stdoutTail !== '') {
            ...
            stdoutBytes = added.used;
            if (added.text.length !== stdoutTail.length)
                stdoutTruncated = true;
        ...
        if (stderrTail !== '') {
            ...
            stderrBytes = added.used;
            if (added.text.length !== stderrTail.length)
                stderrTruncated = true;
        ...
        let stdOutTruncatedFinal = stdoutTruncated;
        let stdErrTruncatedFinal = stderrTruncated;
    ...
    async write(processId, delta, closeStdin = false) {
        const params = { processId, deltaBase64: Buffer.from(delta, 'utf8').toString('base64') };
        if (closeStdin)
