...
export const DEFAULT_DIRECT_LIMITS = {
    readMaxBytes: 262144,
    readMaxLines: 2000,
 ...
 */
export function sandboxExecAvailable(now = Date.now()) {
    if (!SANDBOX_APPLICABLE)
        return false;
    if (sandboxCache !== undefined && now - sandboxCache.at < SANDBOX_TTL_MS)
        return sandboxCache.available;
    let available = false;
    try {
 ...
 */
function fullAccessOf(value) {
    if (value === undefined)
        return DEFAULT_EXEC_POLICY.fullAccess;
    if (value === true || value === false)
        return value;
    throw new DirectOpsError('INVALID_ARGUMENT', 'exec.fullAccess must be a boolean; it is an administrator-only switch and is never inferred from a string', { fullAccess: value });
}
/** Validate the configured exec backend, defaulting to the local sandbox plan. */
function execBackend(value) {
    if (value === undefined)
        return DEFAULT_EXEC_POLICY.backend;
    if (value === 'sandbox-exec' || value === 'codex-app-server')
        return value;
    throw new DirectOpsError('INVALID_ARGUMENT', 'exec.backend must be "sandbox-exec" or "codex-app-server"', { backend: value });
}
...
function buildRoots(configured, writable) {
...
export function readPolicyFile(path) {
    ...
    try {
        ...
        throw new DirectOpsError('INVALID_ARGUMENT', `direct-ops policy file is not valid JSON: ${error.message}`, {
            path: absolute,
        });
    }
}
...
export function resolveDirectOpsPolicy(row, explicitFileConfig) {
    ...
    const limits = {
        ...
        readMaxWindowBytes: positiveInt(merged.limits?.readMaxWindowBytes, 'limits.readMaxWindowBytes', DEFAULT_DIRECT_LIMITS.readMaxWindowBytes),
        writeMaxBytes: positiveInt(merged.limits?.writeMaxBytes, 'limits.writeMaxBytes', DEFAULT_DIRECT_LIMITS.writeMaxBytes),
        execMaxOutputBytes: positiveInt(merged.limits?.execMaxOutputBytes, 'limits.execMaxOutputBytes', DEFAULT_DIRECT_LIMITS.execMaxOutputBytes),
        execTimeoutMs: positiveInt(merged.limits?.execTimeoutMs, 'limits.execTimeoutMs', DEFAULT_DIRECT_LIMITS.execTimeoutMs),
        execMaxTimeoutMs: positiveInt(merged.limits?.execMaxTimeoutMs, 'limits.execMaxTimeoutMs', DEFAULT_DIRECT_LIMITS.execMaxTimeoutMs),
    };
    ...
    if (execEnabled && allowedCommands.length === 0 && !fullAccess) {
        throw new DirectOpsError('INVALID_ARGUMENT', 'exec.enabled requires a non-empty exec.allowedCommands allowlist; there is no "any command" mode. '
    ...
    if (fullAccess && backend !== 'codex-app-server') {
        ...
        throw new DirectOpsError('INVALID_ARGUMENT', 'exec.fullAccess=true requires exec.backend="codex-app-server": the local sandbox-exec backend always writes '
            + 'a confinement profile, so it cannot provide unconfined execution. Set the codex backend, or leave '
            + 'full access off.', { backend, fullAccess: true });
    }
    if (fullAccess && !execEnabled) {
        throw new DirectOpsError('INVALID_ARGUMENT', 'exec.fullAccess=true has no effect while exec.enabled is false; enable exec or clear full access', { fullAccess: true });
    }
    ...
    for (const path of execWriteRoots) {
        if (!readLabels.includes(path) && !writableLabels.includes(path)) {
            throw new DirectOpsError('INVALID_ARGUMENT', `exec.writableRoots entries must also appear in directOps.roots (or writableRoots): ${path}`);
        }
    }
    ...
    if (policyFileReal !== undefined) {
        ...
        const insideWritable = roots.some((root) => root.writable && (policyFileReal === root.real || policyFileReal.startsWith(`${root.real}/`)));
        if (insideWritable) {
    ...
    const validatedCwdRoots = [];
    for (const candidate of requestedCwdRoots) {
        const match = roots.find((root) => candidate === root.real || candidate.startsWith(`${root.real}/`));
        if (match === undefined) {
            throw new DirectOpsError('INVALID_ARGUMENT', `exec.cwdRoots entries must be inside a trusted root: ${candidate}`, { roots: roots.map((root) => root.real) });
        }
        ...
        if (!validatedCwdRoots.includes(real))
            validatedCwdRoots.push(real);
    }
    return {
        ...
        exec: {
            ...
            asyncMaxOutputBytes: positiveInt(merged.limits?.execMaxOutputBytes ?? undefined, 'limits.execMaxOutputBytes', DEFAULT_EXEC_POLICY.asyncMaxOutputBytes),
...
export function resolveAllowedCommand(cmd, policy) {
    ...
    for (const dir of searchPath) {
        ...
        try {
            if (existsSync(candidate) && statSync(candidate).isFile())
                return { path: candidate, usedPath: true };
        }
        catch {
            // keep searching
        }
...
export function isUnderUserDataTree(path) {
...
export function buildSandboxProfile(options) {
    ...
    for (const entry of PROCESS_EXEC_ALLOW)
        lines.push(`(allow process-exec ${entry})`);
    if (binary !== '')
        lines.push(`(allow process-exec (literal ${JSON.stringify(binary)}))`);
    if (policy.exec.network === 'deny')
...
export function sandboxLayerRequested(policy) {
...
export function assertSandboxAvailable(policy) {
    ...
    if (policy.exec.sandbox === 'required') {
        throw new DirectOpsError('SANDBOX_UNAVAILABLE', 'exec.sandbox is "required" but no OS sandbox is available on this host; refusing to run the command '
            + 'unsandboxed. Set exec.sandbox to "preferred" only if you accept an unconfined child.', { platform: process.platform });
    }
    return false;
}
/** Cheap self-description used by dsh_operator_roots. */
export function execFileVersion() {
    try {
