...
export function describePolicy(policy, runtime) {
    ...
    if (!policy.exec.enabled) {
        ...
        notes.push('command execution is ON in administrator full-access mode: the child runs with the same OS user as DSH and '
    ...
    if (backend === 'codex-app-server') {
        if (full) {
            ...
            notes.push('ADMINISTRATOR FULL ACCESS IS ON: commands run with NO OS sandbox. The child may run any bare executable '
            ...
            notes.push('the configured network/filesystem values are NOT in force in this mode; they are reported as '
            ...
            if (policy.exec.enabled) {
                notes.push(policy.exec.writableRoots.length === 0
                ...
                if (policy.exec.filesystem === 'roots') {
                    notes.push('codex readOnly/workspaceWrite permit host-wide READS; the trusted roots do NOT confine command reads, '
...
export function registerDirectOpsTools(server, runtime) {
    ...
    server.registerTool('dsh_start_command', {
        title: 'Start a long command and return a run_id (no agent)',
        description: 'Start one allowlisted command on the DSH host and return a run_id immediately instead of blocking on '
        ...
        annotations: EXEC_ANNOTATIONS,
    }, safe(async (args) => startCommand(args, runtime.policy())));
    server.registerTool('dsh_read_command_output', {
        title: "Read a run's output by run_id (no agent)",
        description: 'Read the accumulated stdout/stderr of one run started with dsh_start_command, plus its status, exit '
    ...
    }, safe(async (args) => readRun(args.run_id, args.since_seq, runtime.policy())));
    server.registerTool('dsh_terminate_command', {
        title: 'Terminate one running command by run_id (no agent)',
        description: 'Stop exactly one run started with dsh_start_command, using the codex app-server terminate call, and '
        ...
        annotations: EXEC_ANNOTATIONS,
    }, safe(async (args) => terminateRun(args.run_id, runtime.policy())));
