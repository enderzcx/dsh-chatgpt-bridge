 ...
 * fact online.
 */
export function effectiveConfinement(plan) {
    ...
    return {
        ...
        network: plan.policy.networkAccess === true ? 'allow' : 'deny',
        filesystem: 'roots',
...
export function resolveCodexBinary(policy) {
    const bin = policy.exec.codexBin;
    if (bin === undefined || bin.trim() === '') {
        throw new DirectOpsError('CODEX_BIN_UNCONFIGURED', 'exec.backend is "codex-app-server" but exec.codexBin is not set; a local administrator must name the '
            + 'codex executable (for example /opt/homebrew/bin/codex). The bridge never guesses it from PATH.', { backend: policy.exec.backend });
    }
    return bin;
}
/**
 ...
 */
export function planCodexSandbox(policy, cwd) {
    ...
    if (filesystem === 'inherit') {
        ...
        throw new DirectOpsError('CODEX_POLICY_UNSUPPORTED', 'exec.filesystem="inherit" has no codex-app-server equivalent; refusing rather than granting '
            + 'danger-full-access. Use exec.filesystem="roots" with exec.writableRoots to grant writes.', { filesystem, backend: policy.exec.backend });
    }
...
function asDirectOpsError(error) {
    ...
    if (error instanceof CodexClientError) {
        ...
        const code = DIRECT_OPS_CODES.includes(error.code)
            ? error.code
            : 'CODEX_EXEC_FAILED';
 ...
 */
async function withClient(policy, onOutputDelta, job) {
 ...
 */
export async function runCommandViaCodex(options, policy) {
...
export async function startCommandViaCodex(options, policy, onOutputDelta) {
    const { path: binary } = resolveAllowedCommand(options.cmd, policy);
    const plan = planCodexSandbox(policy, options.cwd);
    return {
        resolvedBinary: binary,
