...
import { registerDirectOpsTools } from './direct/tools.js';
 ...
 */
const A_PURE_READ = { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false };
/**
 * A local-state write that can neither destroy anything nor start execution:
 * queue reordering and pausing.
 */
const A_LOCAL_WRITE = { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false };
/**
 ...
 */
const A_LOCAL_DESTRUCTIVE = { readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: false };
/**
 ...
 */
const A_DRIVES_AGENT = { readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: true };
/**
 ...
 */
const A_WAITS_ON_AGENT = { readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: true };
...
function errorCodeOfResult(result) {
    try {
...
function registerToolSurfaceFingerprint(server) {
    try {
...
export function createMcpServer(bridge, cfg, log, directOps) {
    ...
    server.registerTool('dsh_health', {
        ...
        let base;
        try {
            base = typeof bridge.health === 'function'
                ? { ...await bridge.health() }
    ...
    server.registerTool('dsh_send_message', {
        ...
        description: 'Continue an EXISTING DSH session through the same agent loop (never a fresh agent). ' +
        ...
        annotations: A_DRIVES_AGENT,
    }, safe(async (args) => bridge.deliverMessage(args.session_id, args.message, args.delivery ?? 'followup')));
    server.registerTool('dsh_list_pending_messages', {
        title: 'List a session\'s pending inbox messages',
        description: 'List the messages DSH has accepted but not yet claimed, with the stable message_id and ' +
        ...
        annotations: A_PURE_READ,
    }, safe(async (args) => bridge.listPendingMessages(args.session_id, args.max_chars)));
    server.registerTool('dsh_promote_pending_message', {
        title: 'Steer a running turn with an already-queued message',
        description: 'Move one message that is still queued for a LATER turn into the current turn as steering, ' +
    ...
    }, safe(async (args) => bridge.promotePendingMessage(args.session_id, args.message_id, args.expected_version)));
    server.registerTool('dsh_edit_pending_message', {
        title: 'Edit a queued message before it is claimed',
        description: 'Replace the text of one still-pending message while preserving its identity, so anything ' +
    ...
    }, safe(async (args) => bridge.editPendingMessage(args.session_id, args.message_id, args.message, args.expected_version)));
    server.registerTool('dsh_withdraw_pending_message', {
        title: 'Withdraw a queued message',
        description: 'Remove one still-pending message from the queue without cancelling the active turn. Use ' +
        ...
        annotations: A_LOCAL_DESTRUCTIVE,
    }, safe(async (args) => bridge.withdrawPendingMessage(args.session_id, args.message_id, args.expected_version)));
    ...
    server.registerTool('dsh_start_goal', {
        ...
        description: 'Creates or continues a supervised DSH Goal for multi-step work: it opens (or reuses) a ' +
    ...
    server.registerTool('dsh_wait_goal', {
        ...
        description: 'Bounded long-poll (default 25s, max 30s) of one DSH session, returning its current ' +
