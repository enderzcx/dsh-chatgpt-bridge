...
import { durableSnapshot, goalMessageOf, hasNonTextBlocks, liveSnapshot, locatePending, messageVersion, pendingView, positionResult, replaceText, targetFor, } from './inbox.js';
...
export class Bridge {
        ...
        this.ctx.on('user-questions/request', async (request, next) => {
            ...
            const viaWeb = this.webOwnsApprovals
                ? Promise.resolve()
                    .then(() => next())
                    .catch((error) => {
                    this.log.warn(`web question answerer unavailable for ${id}: ${error instanceof Error ? error.message : String(error)}`);
                    return undefined;
                })
                : undefined;
            return new Promise((resolve) => {
                ...
                this.log.info(`question ${id} pending for session ${sessionId}${viaWeb === undefined ? '' : ' (web composer + bridge)'}`);
                if (viaWeb !== undefined) {
     ...
     */
    async deliverMessage(sessionId, message, delivery = 'followup') {
        if (message.trim() === '')
        ...
        if (landed === undefined && !this.wasAdmittedSince(agent, messageId, eventsBefore)) {
            throw new BridgeError('DELIVERY_NOT_ACCEPTED', `DSH holds no pending copy of message ${messageId} and the transcript has no record of it; the queue may have been cleared concurrently. Re-read the queue before retrying.`, {
    ...
    wasAdmittedSince(agent, messageId, fromIndex) {
        ...
        return events.some((event) => event.type === 'user/message' && String(event.data.id) === wanted);
    }
    /**
     ...
     */
    async listPendingMessages(sessionId, maxChars) {
        if (sessionId.trim() === '')
            throw new BridgeError('SESSION_REQUIRED', 'session_id is required');
     ...
     */
    async promotePendingMessage(sessionId, messageId, expectedVersion) {
     ...
     */
    async editPendingMessage(sessionId, messageId, message, expectedVersion) {
        if (message.trim() === '')
            throw new BridgeError('EMPTY_MESSAGE', 'message must not be empty');
     ...
     */
    async withdrawPendingMessage(sessionId, messageId, expectedVersion) {
    ...
    static nativeSteerCapable(agent) {
    ...
    requireInbox(sessionId, agent) {
        const inbox = agent.inbox;
        if (inbox === undefined) {
            throw new BridgeError('INBOX_UNAVAILABLE', `session ${sessionId} has no agent inbox; its DSH profile does not mount the agent loop`);
        }
        return inbox;
    }
     ...
     */
    async mutatePending(sessionId, messageId, action, options = {}) {
        if (sessionId.trim() === '')
            throw new BridgeError('SESSION_REQUIRED', 'session_id is required');
        if (messageId.trim() === '')
            throw new BridgeError('MESSAGE_ID_REQUIRED', 'message_id is required');
        this.adopt(sessionId);
        const agent = await this.ensureAgent(sessionId);
        const inbox = this.requireInbox(sessionId, agent);
        ...
        if (options.expectedVersion !== undefined && options.expectedVersion !== '' && options.expectedVersion !== from.version) {
            throw new BridgeError('MESSAGE_VERSION_CONFLICT', `message ${messageId} changed since it was read (expected ${options.expectedVersion}, found ${from.version}); the mutation was refused and nothing was overwritten. Re-read the pending list.`, {
                session_id: sessionId,
                message_id: messageId,
                action,
        ...
        if (action === 'edit') {
            if (hasNonTextBlocks(found.message)) {
                throw new BridgeError('MESSAGE_EDIT_NON_TEXT', `message ${messageId} carries content the bridge cannot re-send faithfully (attachments or non-text blocks); editing it by text would drop that data, so the edit was refused. Withdraw it and send a new message instead.`, { session_id: sessionId, message_id: messageId, queue: this.queueState(agent) });
            }
        ...
        if (found.target !== 'next-turn') {
            throw new BridgeError('MESSAGE_NOT_PROMOTABLE', `message ${messageId} is already in next-step; only a message queued for a later turn can be promoted to steering.`, { session_id: sessionId, message_id: messageId, action, target: found.target, queue: this.queueState(agent) });
        }
        if (agent.status !== 'running') {
            throw new BridgeError('STEER_UNAVAILABLE', `session ${sessionId} is not running (agent status ${JSON.stringify(agent.status)}), so its current turn cannot accept steering. Use dsh_start_goal/dsh_wait_goal to drive the session, or wait until it is running.`, { session_id: sessionId, message_id: messageId, action, agent_status: agent.status, queue: this.queueState(agent) });
        }
        ...
        if (!Bridge.nativeSteerCapable(agent)) {
            throw new BridgeError('STEER_UNAVAILABLE', `session ${sessionId} cannot steer: its agent exposes no native steer()/send() on a live inbox, so the message was left exactly where it was.`, { session_id: sessionId, message_id: messageId, action, reason: 'NATIVE_STEER_MISSING', queue: this.queueState(agent) });
        }
        ...
        try {
            ...
            if (pendingNow !== undefined) {
                ...
                throw new BridgeError('STEER_REDELIVERY_FAILED', `steering ${messageId} was rejected (${detail}), but DSH still holds it in ${pendingNow.target}; it is queued exactly once and was not lost.`, {
                    session_id: sessionId,
                    message_id: messageId,
                    action,
                    reason: detail,
            ...
            if (this.wasAdmitted(agent, messageId)) {
                ...
                throw new BridgeError('STEER_REDELIVERY_FAILED', `steering ${messageId} was rejected after it was already claimed (${detail}); it is in the transcript exactly once.`, {
                    session_id: sessionId,
                    message_id: messageId,
                    action,
                    reason: detail,
            ...
            if (recovery === 'restored') {
                throw new BridgeError('STEER_REDELIVERY_FAILED', `steering ${messageId} was rejected (${detail}). The message was not delivered, so it has been restored to its previous queue position and remains queued exactly once.`, {
                    session_id: sessionId,
                    message_id: messageId,
                    action,
                    reason: detail,
            ...
            throw new BridgeError('STEER_RECOVERY_REQUIRED', `steering ${messageId} was rejected (${detail}) and the message could not be put back; it is NOT queued now. Re-send it with dsh_send_message.`, {
                session_id: sessionId,
                message_id: messageId,
                action,
                reason: detail,
        ...
        return {
            ...(after === undefined
                ? { ...from, target: 'next-step', delivery: 'steer', index: steerIndex }
                : positionResult(after.message, after.target, after.index)),
            action,
            previous: from,
            queue: this.queueState(agent),
     ...
     */
    refuseProtectedGoalMessage(sessionId, message, action) {
        ...
        if (goal.stale) {
            throw new BridgeError('GOAL_MESSAGE_STALE', `message ${String(message.id)} is a superseded Goal control message (rev ${goal.revision} of ${record?.revision}); it must not be edited, withdrawn or sent back into the current turn. Use ${target} to move the Goal, or withdraw it only if you intend to drop it.`, { session_id: sessionId, message_id: String(message.id), action, goal_id: goal.goal_id, message_revision: goal.revision, current_revision: record?.revision });
        }
        throw new BridgeError('GOAL_MESSAGE_PROTECTED', `message ${String(message.id)} is a supervised-Goal control message; ${action} would bypass the Goal revision rules. Use ${target} to revise, defer or resume the Goal instead.`, { session_id: sessionId, message_id: String(message.id), action, goal_id: goal.goal_id, revision: goal.revision });
    }
    /**
     ...
     */
    staleMessageError(agent, sessionId, messageId, action) {
        const details = { session_id: sessionId, message_id: messageId, action, queue: this.queueState(agent) };
        if (this.wasAdmitted(agent, messageId)) {
            return new BridgeError('MESSAGE_ALREADY_ADMITTED', `message ${messageId} already entered a step and is part of the transcript; it can no longer be promoted, edited or withdrawn.`, details);
        }
        if (this.eventsOf(agent).some((event) => event.type === 'agent/inbox/spliced' || event.type === 'user/message')) {
            return new BridgeError('MESSAGE_NOT_PENDING', `message ${messageId} is not pending in this inbox: it was already claimed, withdrawn, or discarded by a cancellation. Re-read the pending list before retrying.`, details);
        }
    ...
    wasAdmitted(agent, messageId) {
        const wanted = String(messageId);
        return this.eventsOf(agent).some((event) => event.type === 'user/message' && String(event.data.id) === wanted);
    }
    ...
    async startGoal(input) {
        ...
        await this.deliverMessage(sessionId, this.controlMessage(record, input.goal, input.plan, record.revision === 1 ? 'start' : 'revise'), 'followup');
        if (input.request_id !== undefined && input.request_id !== '') {
            this.goalRequests.set(input.request_id, { sessionId, fingerprint });
        }
        const view = await this.loadView(sessionId);
    ...
    async updateGoal(input) {
        ...
        await this.deliverMessage(sessionId, this.controlMessage(record, input.goal ?? record.goal, input.plan ?? record.plan, intent, resumeIds), 'followup');
        if (input.request_id !== undefined && input.request_id !== '') {
            this.goalRequests.set(input.request_id, { sessionId, fingerprint });
        }
        const view = await this.loadView(sessionId);
    ...
    observeGoal(sessionId, view, status, options = {}) {
    ...
    goalFields(sessionId, view, status, options = {}) {
    ...
    async goalSnapshot(sessionId, view, status, waitedMs, waitSeconds, options = {}) {
