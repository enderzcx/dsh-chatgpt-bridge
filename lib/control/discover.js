...
function envOf(hooks) {
    return hooks.env ?? process.env;
}
function platformOf(hooks) {
...
export function defaultProfileDirs(env, platform) {
    const path = pathOf(platform);
    if (platform === 'win32') {
...
export function parseTunnelCommandLine(commandLine) {
...
export function healthBaseFromListenAddr(addr) {
...
function parseCimProcessJson(raw) {
    const text = raw.trim();
    if (text === '')
        return [];
    try {
...
function defaultListRunningProcesses(platform) {
    try {
        if (platform === 'win32') {
            ...
            for (const row of parseCimProcessJson(result.stdout ?? '')) {
                ...
                if (!Number.isInteger(pid) || pid <= 0 || executablePath === '')
                    continue;
        ...
        for (const line of raw.split(/\r?\n/)) {
            ...
            if (trimmed === '' || !trimmed.includes('tunnel-client'))
                continue;
...
function defaultListRunningExecutables(platform) {
    return defaultListRunningProcesses(platform)
        .map((row) => row.executablePath)
        .filter((path) => path !== '');
}
function resolveRunningProcesses(hooks, platform) {
...
export function discoverExecutable(hooks = {}) {
    ...
    if (fromWellKnown !== undefined)
        return { path: fromWellKnown, source: 'well-known' };
    ...
    if (fromRunning !== undefined)
        return { path: fromRunning, source: 'running-process' };
    return undefined;
}
export function resolveTunnelClientExecutable(configured) {
...
function readProfileHints(hooks, profilePath) {
    const read = hooks.readFileSync ?? ((p) => readFileSync(p, 'utf8'));
    let text = '';
    try {
        if (hooks.existsSync === undefined) {
            try {
                if (statSync(profilePath).size > MAX_PROFILE_BYTES)
                    return {};
            }
            catch {
                return {};
    ...
    if (text.length > MAX_PROFILE_BYTES)
        text = text.slice(0, MAX_PROFILE_BYTES);
...
function listProfileFiles(hooks, dir, platform) {
    const readDir = hooks.readdirSync ?? ((p) => readdirSync(p));
    const path = pathOf(platform);
    let names = [];
    try {
        names = readDir(dir);
...
export function discoverExistingRuntime(hooks = {}) {
    const env = envOf(hooks);
    const platform = platformOf(hooks);
    const out = {};
    ...
    for (const dir of dirs) {
        for (const file of listProfileFiles(hooks, dir, platform)) {
            ...
            if (PROFILE_NAMES_PREFERRED.includes((file.replace(/\\/g, '/').split('/').pop() ?? '').toLowerCase()))
                break;
            if (hints.tunnelId !== undefined)
                break;
        }
        if (chosen?.tunnelId !== undefined)
            break;
    }
    if (chosen !== undefined) {
        out.profilePath = chosen.profilePath;
        if (chosen.profileName !== undefined)
            out.profileName = chosen.profileName;
        if (chosen.tunnelId !== undefined)
            out.tunnelId = chosen.tunnelId;
        if (chosen.controlPlaneBaseUrl !== undefined)
            out.controlPlaneBaseUrl = chosen.controlPlaneBaseUrl;
        if (chosen.runtimeApiKeyAvailable !== undefined)
            out.runtimeApiKeyAvailable = chosen.runtimeApiKeyAvailable;
        if (chosen.healthBaseUrl !== undefined)
            out.healthBaseUrl = chosen.healthBaseUrl;
    }
    ...
    if (processes.some((row) => row.proxyFlag === true))
        out.proxyInUse = true;
    const proxy = parseProxyEnv(env.HTTPS_PROXY) ??
