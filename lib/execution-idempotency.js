...
export function idempotencyKindFor(toolName, command) {
    const cmd = command ?? '';
    if (hasShellControlOperator(cmd))
        return undefined;
    if (/\bnpm\s+publish\b/i.test(cmd) || toolName === 'npm.publish')
        return 'npm_publish';
    if (/\bgh\s+release\b/i.test(cmd) || toolName === 'github.release')
...
export class ExecutionIdempotencyManager {
    evidenceCache = new Map(); // fingerprint -> evidence
    cap;
     ...
     */
    recordSuccess(fingerprint, input) {
    ...
    listEvidence(filter) {
        return [...this.evidenceCache.values()].filter((evidence) => {
            if (filter?.sessionId !== undefined && !(evidence.sessionIds ?? []).includes(filter.sessionId))
                return false;
