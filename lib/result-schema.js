...
import { foldGoalFacts } from './goal-facts.js';
...
const STORE_NAMED_REFS = new Set(ENV_CREDENTIAL_REFS);
...
function extractTestMetrics(facts) {
    let pass = 0;
    let fail = 0;
    let skip = 0;
    const suites = [];
    for (const tool of facts.tools) {
        if (tool.command && /\b(?:test|jest|vitest|mocha|node\s+--test)\b/i.test(tool.command)) {
            ...
            if (passMatch)
                pass = Math.max(pass, parseInt(passMatch[1] ?? '0', 10));
            const failMatch = text.match(FAIL_COUNT_REGEX);
            if (failMatch)
                fail = Math.max(fail, parseInt(failMatch[1] ?? '0', 10));
            const skipMatch = text.match(SKIP_COUNT_REGEX);
            if (skipMatch)
                skip = Math.max(skip, parseInt(skipMatch[1] ?? '0', 10));
...
function collectAllChangedFiles(events) {
    const seen = new Set();
    const paths = [];
    ...
    for (const event of events) {
        ...
        if (!data || typeof data.name !== 'string' || !EDIT_TOOL_NAMES.has(data.name))
            continue;
        try {
...
export function extractCommitShas(text) {
    const out = [];
...
export function extractTagNames(command, resultText) {
    const out = [];
    if (command) {
        const fromCmd = command.match(TAG_COMMAND);
        if (fromCmd?.[1] && fromCmd[1] !== '-a' && fromCmd[1] !== '--annotate')
            out.push(fromCmd[1]);
    }
    if (resultText) {
        const fromResult = resultText.match(TAG_CREATED);
        if (fromResult?.[1])
            out.push(fromResult[1]);
    }
    return unique(out);
}
function parseSize(text) {
    ...
    if (bytes)
        return parseInt(bytes[1] ?? '0', 10);
    return undefined;
}
export function extractArtifacts(tools) {
    const seen = new Set();
    const artifacts = [];
    ...
    for (const tool of tools) {
        ...
        if (file?.[1])
            add(file[1], extra);
        if (tool.filePath && ARTIFACT_FILE.test(tool.filePath))
            add(tool.filePath, extra);
    }
    return artifacts;
}
function collectCredentialKeysFromYaml(text) {
    const keys = [];
...
function upsertCredential(byRef, ref, source, available) {
    if (!available)
        return;
    const next = { credentialAvailable: true, credentialRef: ref, credentialSource: source };
...
function scanSecretLeak(facts, changedFiles) {
    const warnings = [];
    for (const tool of facts.tools) {
        const text = `${tool.command ?? ''}\n${tool.resultText ?? ''}`;
...
export function buildResultSchema(input) {
    ...
    for (const tool of facts.tools) {
        ...
        if (tool.kinds.includes('git_push'))
            pushVerified = true;
        if (tool.kinds.includes('npm_publish'))
            npmVerified = true;
        if (tool.kinds.includes('github_release'))
            githubVerified = true;
